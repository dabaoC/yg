;(function($){
	$.fn.carousel = function(param){//param 参数
		...
		$(prev).off("click").on("click",function(){
			...
			if(on.prev().is("li")){
				...
				// delay()方法是 slideUp() 和 .fadeIn()之间延时
				on.prev().stop(true,true).addClass("on").fadeIn(animateTime);
			}else{
				$.switchIndex($(carousel).children("li").index(list[list.length-1]));
				$(list[list.length-1]).stop(true,true).addClass("on").fadeIn(animateTime);
			}
		...
		$(next).off("click").on("click",function(){
			...
			if(on.next().is("li")){
				...
				on.next().stop(true,true).addClass("on").fadeIn(animateTime);
			}else{
				$.switchIndex($(carousel).children("li").index(list[0]));
				$(list[0]).stop(true,true).addClass("on").fadeIn(animateTime);
			}
		});

		$(indexList).off("mouseenter").on("mouseenter",function(){
			if ($(this).attr("class") != "index") {
				...
				on.stop(true,true).fadeOut(animateTime).removeClass("on");
				$(list[index]).stop(true,true).addClass("on").fadeIn(animateTime);
			}
		});
